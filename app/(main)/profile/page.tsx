  ...
  return (
    <div className="py-16">
      <div className="container mx-auto py-8">
        <div className="grid grid-cols-4 sm:grid-cols-12 gap-6 px-4 my-8">
          <div className="col-span-4 sm:col-span-3 dark:border border-gray-400">
            <div className="shadow rounded-lg p-6">
              <div className="flex flex-col items-center">
                <Image
                  ...
                  width={100}
                  height={100}
                  className="w-32 h-32 bg-gray-300 dark:bg-gray-700 rounded-full mb-4 shrink-0"
                />
                <h1 className="text-xl dark:text-zinc-400 font-bold">
                  {user?.name}
                </h1>
                <p className="text-gray-700 dark:text-gray-500">{user?.email}</p>
                  ...
                  <ul>
                    {
                      user?.skills?.map((skill) => 
                        <li className="mb-2" key={skill.id}>{skill?.text}</li>
                      )
                    }
                  </ul>
                </div>
                      ...
                      {user?.workExperience
                        ?
                          user?.workExperience?.map((exp) => 
                            <div 
                              key={exp.id}
                              className="mb-6 flex w-full dark:border border-gray-400"
                            >
                              <div className="flex justify-between flex-wrap gap-2 w-full">
                                  <span className="text-gray-700 font-bold">
                                    {exp.jobTitle ?? "no experience yet"}
                                  </span>
                                  <p>
                              ...
                              <p className="mt-2">
                                {exp?.duties || "Owner of this website"}
                              </p>
                            </div>
                          )
                        : "No experience yet"
