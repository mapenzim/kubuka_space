...
export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const NavItem = ({ href, icon, label }: { href: string; icon: React.ReactNode; label: string }) => (
    <Button 
      asChild 
      variant="ghost" 
      color="gray" size="3" className="w-full justify-start items-start cursor-pointer"
  ...
  return (
    ...
    <Flex className="h-screen w-full overflow-hidden bg-(--gray-a2)">
      ...
      <Flex 
        direction="column" 
        className="hidden w-64 shrink-0 border-r border-(--gray-a6) bg-(--color-panel) md:flex"
      >
        {/* Title */}
        <Box p="5">
        ...
        <Flex direction="column" gap="1" p="4" className="flex-1 overflow-y-auto">
          <Text size="1" weight="bold" color="gray" mb="2" className="uppercase tracking-wider">
            Overview
          </Text>
          <NavItem 
            href="/admin" 
          ...
          <NavItem 
            href="/admin/posts" 
            label="All Posts" 
            icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2v4a2 2 0 0 0 2 2h4"/><path d="M10.4 12.6a2 2 0 1 1 3 3L8 21l-4 1 1-4Z"/><path d="M18 21v-8a2 2 0 0 0-2-2h-1.5"/><path d="M4 11V4a2 2 0 0 1 2-2h8l6 6v3"/></svg>} 
          />
          <NavItem 
            href="/admin/users" 
            label="Users" 
          ...
          <NavItem 
            href="/admin/messages" 
            label="Messages" 
            icon={<MailPlus width={18} height={18} />} 
          />
        </Flex>
      ...
      <Flex direction="column" className="flex-1 overflow-hidden">
        {/* Header/Topbar inside main pane (Optional, good for mobile menus or breadcrumbs) */}
        <Flex align="center" className="h-16 shrink-0 border-b border-(--gray-a6) bg-(--color-panel) px-6">
           <Heading as="h3" size="4">Dashboard Overview</Heading>
        </Flex>
